...
<main class="flex-grow p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <h4 class="text-xl font-semibold text-gray-800 dark:text-gray-100">Advocates</h4>
        <span class="text-sm text-gray-500 dark:text-gray-400">{{ count($advocates) }} total</span>
    </div>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                                        <div class="flex items-center justify-end gap-3">
                                            ...
                                            <form action="{{ route('admin.update.user.status', $s->id) }}" method="POST" class="inline"
                                                onsubmit="return confirm('Toggle this advocate\'s status?')">
                                                @csrf
                                                <button type="submit"
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-6 py-10 text-center text-sm text-gray-500 dark:text-gray-400">
                                        <i class="mgc_building_2_line text-4xl mb-2 block opacity-30"></i>
                                        No advocates found.
                                    </td>
                                </tr>
                            @endforelse
