...
@section('content')
    <main class="flex-grow p-6">
        <div class="">
            <div class="my-2">
                <div class="card p-6">
                    ...
                    <div class="flex flex-col gap-3">
                        <div>
                            <label for="project-description" class="mb-2 block">Question <span class="text-red-500">*</span></label>
                            <textarea id="question" name="question" class="form-input" rows="8">{{ $question->question }}</textarea>
                        </div>
                        <div>
                            <label for="project-name" class="mb-2 block">Marks</label>
                            <input type="text" id="marks" name="marks" value="{{ $question->marks }}" class="form-input" placeholder="Enter Marks">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div>
                <div class="col-span-1 flex flex-col gap-6" id="options-container" >
                    @foreach ($question->options as $index => $option)
                        <div class="card p-6 my-1">
                            <div class="flex flex-col gap-3">
                                <div class="flex justify-between">
                                    <label class="mb-2 block">Option {{ $loop->iteration }}</label>
                                    <a href="javascript:void(0);" class="remove-option text-red-500"> 
                                        <i class="mgc_delete_line text-xl"></i> 
                                    </a>
                                </div>
                                <input type="text" value="{{ $option->option }}" class="form-input option-input">
                            </div>
                            <div class="mt-4">
                                <label class="mb-2 block">Is Correct <span class="text-red-500">*</span></label>
                                <div class="flex">
                                    <input type="checkbox" name="correct_option" class="form-checkbox correct-option" data-index="{{ $index }}" {{ $option->is_correct ? 'checked' : '' }}>
                ...
                <button id="add-option" type="button" class="mt-3 bg-red-100 text-white px-4 py-2 rounded">Add Option</button>
            </div>
            
            <div class="lg:col-span-4 mt-5">
                <div class="flex justify-start gap-3">
                    <button type="button" class="bg-red-500 text-white px-4 py-2 rounded">Cancel</button>
                    <button id="save-question" type="button" class="bg-green-500 text-white px-4 py-2 rounded">Save</button>
                </div>
            </div>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        document.querySelector("#save-question").addEventListener("click", function () {
            ...
            document.querySelectorAll(".option-input").forEach((input, index) => {
                options.push({
                    option: input.value,
                    is_correct: document.querySelector(`.correct-option[data-index='${index}']`).checked ? 1 : 0
                });
            });
            formData.append("options", JSON.stringify(options));
            fetch("{{ route('advocate.exams.question.update', [$exam->id, $question->id] ) }}", {
                method: "POST",
                body: formData,
                headers: {
