            ...
            <div id="all" class="content space-y-20 gap-6 md:container">
                ...
                <div style="margin-bottom: 250px" class="relative flex lg:h-[600px] overflow:hidden">
                    ...
                    <div class="w-full h-full relative">
                        <!-- Toggle Button -->
                        <div onclick="toggleDetails()" style="color: #4F92FE"
                            class="toggler absolute -top-10 -left-100 h-10 w-1000 shadow bg-danger-600 overflow-hidden rounded-xl p-4 cursor-pointer">
                            Show more details
                            <i class="fa fa-eye"></i>
                        ...
                        <img class="w-full h-full object-cover rounded-2xl -z-20"
                            src="{{$assets.$event->getRawOriginal('image')}}" alt="image here">
                        {{-- <img class="w-full h-full object-cover rounded-2xl -z-20"
                            src="/{{$event->getRawOriginal('image')}}" alt="image here"> --}}
                        <div
                        ...
                        <div
                            class="toggleUp hidden md:block absolute -top-10 -right-10 h-24 w-52 shadow bg-gray-200 dark:bg-dark2 overflow-hidden rounded-xl p-4">
                        ...
                        <!-- Event Detail Card -->
                        <div
                            class="toggleUp absolute top-1/4 md:-left-[10%] h-2/3 w-full md:w-[45%] bg-gray-200 dark:bg-dark2 rounded-2xl md:rounded-3xl p-6 dark:text-white">
                            ...
                            <div>
                                ...
                                <div class="flex flex-col gap-4">
                                    <p class="dark:text-gray-100">{{
                                        \Carbon\Carbon::parse($event->start_datetime)->format('d M') }} - {{
                                        \Carbon\Carbon::parse($event->end_datetime)->format('d M Y') }} | {{
                                        $event->duration }} hours</p>
                                    <span class="rounded-full w-fit px-2 text-gray-100 bg-gray-600 uppercase">{{
                                        $event->location }}</span>
                                </div>
                                <button onclick="showEventDetail({{ json_encode($event) }})"
    ...
    <section id="eventDetailContainer">
        <div id="eventDetail" class="hidden max-w-4xl mx-auto">
            ...
            <div id="detailContent" class="bg-secondary dark:bg-dark2 dark:text rounded-xl p-8 shadow-md">
                ...
                <button
                    class="register-now-button mt-6 w-full bg-primary text-white py-3 rounded-lg hover:bg-opacity-50 transition-all">
                    Register Now
                </button>
            </div>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function showSection(id) {
            ...
            const anchor = tab.querySelector("a"); // Get the <a> inside <li>
            tab.classList.toggle("bg-primary", anchor.getAttribute("href") === `#${id}`);
            tab.classList.toggle("text-white", anchor.getAttribute("href") === `#${id}`);
        });
        }

        const currentHash = window.location.hash.substring(1);
        showSection(currentHash || "all");
