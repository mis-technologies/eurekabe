...
class BlogSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // Helper: find school id by partial name
        $schoolId = function (string $name) use ($schools): ?int {
            $match = $schools->first(fn($s) => str_contains($s->name, $name));
            return $match?->id;
        };
        ...
        foreach ($articles as $article) {
            ...
            $blog = Blog::firstOrCreate(
                ...
                [
                    'title'       => $article['title'],
                    'category_id' => $cat->id,
                    'school_id'   => $schoolId($article['school'] ?? ''),
                    'image'       => $article['image'],
            ...
            // Backfill school_id on existing rows that were seeded before the column existed
            if (!$blog->wasRecentlyCreated && !$blog->school_id && isset($article['school'])) {
                $blog->update(['school_id' => $schoolId($article['school'])]);
            }
        }
    }
}
