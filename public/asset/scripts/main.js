document.addEventListener("DOMContentLoaded", () => {
    ...
    //         const selects = form.querySelectorAll("select[required]:not(:disabled)");
    //         let allFilled = true;
    ...
    //                 // AI changes: Save first form data before transitioning
    //                 firstForm.querySelectorAll("input", "select").forEach(input => {
    //                     firstFormData[input.id] = input.value;
    //                 });
    ...
    //                 let secondFormData = {};
    //                 secondForm.querySelectorAll("input, select").forEach(input => {
    //                     secondFormData[input.id] = input.value;
    //                 });
    ...
    //                 // AI changes: Send data to the backend
    //                 fetch("advocate/register", {  // Replace with actual endpoint
    //                     method: "POST",
    //                     headers: {
    ...
    //                         // AI changes: Show popup after successful submission
    //                         popUp();
    //                     })
    //                     .catch(error => {
    ...
    //             // AI changes: Restore first form values when navigating back
    //             Object.keys(firstFormData).forEach(id => {
    //                 const input = firstForm.querySelector(`#${id}`);
    //                 if (input) input.value = firstFormData[id];
    //             });
    ...
    function initializeRegistrationPage() {
        ...
        if (firstForm && secondForm && imageContainer && formContainer) {
            ...
            const transitioned = () => {
                if (!continueButton.disabled) {
                    // Save first form data before transitioning
                    firstForm.querySelectorAll("input, select").forEach(input => {
                        firstFormData[input.id] = input.value;
                    });
        ...
        if (dot1 && dot2) {
            dot1.addEventListener("click", () => {
                ...
                // Restore first form values when navigating back
                Object.keys(firstFormData).forEach(id => {
                    const input = firstForm.querySelector(`#${id}`);
                    if (input) input.value = firstFormData[id];
                });
    ...
    //     let firstFormData = {}; // AI changes: Object to store first form data
    ...
    //     if (firstForm && secondForm && imageContainer && formContainer) {
    //         // Add input event listeners to first form
    //         firstForm.querySelectorAll("input[required], select[required]").forEach((element) => {
    //             element.addEventListener("change", () => toggleButtonState(firstForm, continueButton));
    //             element.addEventListener("input", () => toggleButtonState(firstForm, continueButton));
    //         });

    //         // Add input event listeners to second form
    //         secondForm.querySelectorAll("input[required], select[required]").forEach((element) => {
    //             element.addEventListener("change", () => toggleButtonState(secondForm, applyButton));
    //             element.addEventListener("input", () => toggleButtonState(secondForm, applyButton));
    //         });
    ...
    //                 // Remove the fade-in class after animation completes
    //                 secondForm.addEventListener("animationend", () => {
    //                     secondForm.classList.remove("fade-in");
    //                 }, { once: true });
    //             }
    //         };
    ...
    //         // AI changes: Handle form submission on clicking applyButton
    //         applyButton.addEventListener("click", () => {
    //             if (!applyButton.disabled) {
    //                 // Collect second form data
    //                 let secondFormData = {};
    ...
    //     // Function to handle dots navigation
    //     if (dot1 && dot2) {
    //         dot1.addEventListener("click", () => {
    //             if (secondForm.classList.contains("hidden")) return;
    ...
    function initializeBgParticleMovement() {
        ...
        particles.forEach((particle) => {
            ...
            particle.style.left = `${Math.random() * (containerWidth - particle.clientWidth)
                }px`;
            particle.style.top = `${Math.random() * (containerHeight - particle.clientHeight)
                }px`;
