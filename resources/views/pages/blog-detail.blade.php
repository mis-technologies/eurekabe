    ...
    <ul
        ...
        <li class="mini-nav cursor-pointer font-bold capitalize hover:text-primary opacity-80 whitespace-nowrap">
            <a href="{{ route('blogs', ['category_id' => $category->id]) }}"
                class="tab {{ $category->id == $blog->category_id ? 'active' : '' }}">
                {{ $category->name }}
            </a>
        </li>
        @endforeach
...
<div
    class="flex flex-col items-center gap-8 w-full max-w-[1024px] mx-auto px-6 dark:bg-dark-card dark:text-white pb-10">
    <!-- Back Button -->
    <div class="w-full">
        <a href="{{route('blogs')}}"
    ...
    <!-- Image -->
    <img style="height: 100%; width: 100%" src="{{$assets.$blog?->image}}" alt="${article.title}"
        class="w-full h-auto md:w-auto md:h-80 object-cover rounded-lg" />

    <!-- Content -->
    <div style="width:100%; max-width:100%; overflow-x:hidden; padding:0 1rem; box-sizing:border-box;">
